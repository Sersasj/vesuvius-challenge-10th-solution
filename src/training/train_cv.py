...
from src.data.dataset import VesuviusDataset, CutMixCollator
import os
...
def create_dataloaders(args, fold_dir):
    ...
    # CutMix collator
    collate_fn = None
    if getattr(args, 'use_cutmix', False) and args.batch_size >= 2:
        collate_fn = CutMixCollator(prob=args.cutmix_prob, max_cube_ratio=args.cutmix_ratio)
        print(f"Using CutMix: prob={args.cutmix_prob}, ratio={args.cutmix_ratio}")

    loader_args = dict(num_workers=args.num_workers, pin_memory=True,
                       prefetch_factor=args.prefetch_factor, worker_init_fn=worker_init_fn)

    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn, **loader_args)
...
def get_checkpoint_config(args):
    ...
    if monitor == "auto":
        monitor = "val/leaderboard_score" if args.compute_leaderboard_metrics else "val/loss"
        mode = "max" if args.compute_leaderboard_metrics else "min"
    if mode == "auto":
        mode = "min" if "loss" in monitor.lower() else "max"

    filename = args.ckpt_filename
    if filename == "auto":
        filename = ("best-{epoch:02d}-{val_leaderboard_score:.4f}-{val_dice:.4f}"
                    if monitor == "val/leaderboard_score"
                    else "best-{epoch:02d}-{val_loss:.4f}-{val_dice:.4f}")
...
def train_fold(args, fold_idx, train_df, val_df):
    ...
    if wandb.run:
        wandb.finish()
    wandb_logger = (WandbLogger(project=args.wandb_project, name=f"{args.model_type}_all_losses_fold_{fold_idx}",
...
def parse_args():
    ...
    p.add_argument('--model_type', type=str, default='unet')
    ...
    p.add_argument('--cutmix_prob', type=float, default=0.3)
    p.add_argument('--cutmix_ratio', type=float, default=0.5)
    ...
    # Regularization
    p.add_argument('--drop_path_rate', type=float, default=0.0, help="Drop path rate for Primus/PrimusV2 (paper uses 0.2)")
