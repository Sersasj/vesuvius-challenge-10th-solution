...
from src_deformnet_stage.models.lightning_module import SegmentationModule
from src_deformnet_stage.data.dataset import VesuviusDataset
import os
...
def create_model(args):
    ...
    model = SegmentationModule(
        ...
        n_steps = getattr(args, 'n_steps', 6),
        max_v = getattr(args, 'max_v', 1.5),
        max_topo_offset=getattr(args, 'max_topo_offset', 1.0),
        lambda_jac=getattr(args, 'lambda_jac', 0.3),
        lambda_smooth=getattr(args, 'lambda_smooth', 0.05),
        lambda_sparse=getattr(args, 'lambda_sparse', 0.1),
        lambda_tv = getattr(args, 'lambda_tv', 0.02),
        lambda_boundary=getattr(args, 'lambda_boundary', 0.1),
        ...
        sigma = getattr(args, 'sigma', 2),
        surface_dice_iterations=getattr(args, 'surface_dice_iterations', 5),
        ce_top_k=getattr(args, 'ce_top_k', 1.0),
        oof_threshold=getattr(args, 'oof_threshold', 0.3),
        oof_aug_sigma=getattr(args, 'oof_aug_sigma', 0.1),
        apply_gaussian_oof=getattr(args, 'apply_gaussian_oof', True),
...
def train_fold(args, fold_idx, train_df, val_df):
    ...
    if wandb.run:
        wandb.finish()
    wandb_logger = (WandbLogger(project=args.wandb_project, name=f"deformnet_all_losses_fold_{fold_idx}",
...
def parse_args():
    ...
    # DeformNet / Topology-aware deformation configs
    p.add_argument('--n_steps', type=int, default=6,
                   help="Number of deformation integration steps")
    p.add_argument('--max_v', type=float, default=1.5,
                   help="Maximum velocity magnitude for deformation field")
    p.add_argument('--max_topo_offset', type=float, default=1.0,
                   help="Maximum allowed topological offset")

    # Deformation regularization losses
    p.add_argument('--lambda_jac', type=float, default=0.3,
                   help="Jacobian determinant regularization weight")
    ...
    p.add_argument('--lambda_boundary', type=float, default=0.1,
                   help="Boundary-aware loss weight")

    # Surface / topology metrics
    p.add_argument('--surface_dice_iterations', type=int, default=5,
                   help="Number of iterations for surface Dice computation")
    p.add_argument('--ce_top_k', type=float, default=1.0,
                   help="Top-k ratio for hard voxel mining in CE loss")

    # OOF (Out-Of-Fold) related configs
    p.add_argument('--oof_threshold', type=float, default=0.3,
                   help="Threshold for OOF-based masking")
    p.add_argument('--oof_aug_sigma', type=float, default=0.1,
                   help="Gaussian noise sigma for OOF augmentation")
    p.add_argument('--apply_gaussian_oof', action='store_true', default=True,
                   help="Apply Gaussian smoothing to OOF predictions")

    # Post-processing / inference
    p.add_argument('--threshold', type=float, default=0.5,
                   help="Final probability threshold for segmentation output")

    # Gaussian kernel parameters
    p.add_argument('--kernel_size', type=int, default=11,
                   help="Kernel size for Gaussian filtering")
